...
export const logout_service = async (req: Request, res: Response) => {
    try {
        ...
        if (!name_service || !email) {
            return res.status(400).json({ message: 'Both "name_service" and "email" fields are required.' });
        }
        ...
        if (!serviceMap || serviceMap.get(name_service) === undefined) {
            return res.status(404).json({ message: `Service "${name_service}" not found for user "${email}".` });
        }
        ...
        return res.status(200).json({ message: `Service "${name_service}" has been successfully logged out.` });
    } catch (error) {
