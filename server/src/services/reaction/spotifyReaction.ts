...
export const createPlaylist_spotify = async (email: string, actionResult?: any): Promise<SpotifyResponse | null> => {
  try {
    ...
    const playlistName = `AREA Playlist - ${new Date().toLocaleString()}`;
    ...
    console.log('✅ Playlist créée avec succès:', response.body.external_urls.spotify);
    return {
    ...
    throw new Error(
      `Erreur lors de la création de la playlist: ${error instanceof Error ? error.message : 'Erreur inconnue'}`
    );
  }
};