...
export const sendGifDiscord = async (email: string, actionResult?: any): Promise<DiscordResponse | null> => {
  try {
    ...
    throw new Error(
      `Erreur lors de l'envoi du GIF: ${error instanceof Error ? error.message : 'Erreur inconnue'}`
    );
  }
};