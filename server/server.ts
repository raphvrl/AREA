...
const spotifyApi = new SpotifyWebApi({
  ...
  redirectUri: 'https://localhost:5000/api/auth/spotify/callback'
});
...
app.get('/api/auth/spotify/callback', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.redirect(`http://localhost:3000/login?spotify_token=${access_token}&firstName=Spotify&lastName=User&isFirstLogin=true`);
  } catch (error) {
